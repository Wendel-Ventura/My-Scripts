...
colunas_numericas = ['CARGA_HORARIA_MENSAL','CARGA_HORARIA_SEMANAL','SALARIO_ATUAL','CODIGO_DA_EMPRESA',
...
colunas_inteiras = ['CODIGO_DA_EMPRESA','MATRICULA_ESOCIAL',
...
df[colunas_inteiras] = df[colunas_inteiras].fillna(0)
...
for col3 in colunas_txt:
    df[col3] = df[col3].astype(str)
   

for col5 in colunas_txt:
   df[col5] = df[col5].replace('nan', np.nan)
   
for col6 in colunas_txt:
   df[col6] = df[col6].fillna('-')
