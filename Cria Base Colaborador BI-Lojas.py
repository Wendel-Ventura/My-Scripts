...
def definir_geracao(ano):
    if 1928 <= ano <= 1945:
        return '1- Silent Generation (1928 - 1945)'
    elif 1946 <= ano <= 1964:
        return '2- Baby Boomer (1946 - 1964)'
    elif 1965 <= ano <= 1980:
        return '3- Geração X (1965 - 1980)'
    elif 1981 <= ano <= 1996:
        return '4- Millennials (1981 - 1996)'
    elif 1997 <= ano <= 2012:
        return '5- Geração Z (1997 - 2012)'
    else:
        return '6- Geralção Alfa'
...
bd['Data Admissão'] = pd.to_datetime(bd['Data Admissão'], errors='coerce')
bd['Data de Rescisão'] = pd.to_datetime(bd['Data de Rescisão'], errors='coerce')

data_atual = datetime.now()

bd['tempo_de_casa_meses'] = ((bd['Data de Rescisão'] - bd['Data Admissão']).dt.days // 30).fillna((data_atual - bd['Data Admissão']).dt.days // 30)

# Criando a nova coluna de agrupamento
bd['tempo_de_casa_grupo'] = pd.cut(bd['tempo_de_casa_meses'], 
                                    bins=[-float('inf'), 3, 6, 12, 24, float('inf')],
                                    labels=['1- 0-3 meses', '2- 4-6 meses', '3- 7-11 meses', '4- 12-24 meses', '5- Acima de 24 meses'])

def substituir_cargos(cargo):
    cargo = cargo.replace('GTE LOJA I','GTE LOJA')
    cargo = cargo.replace('GTE LOJAI','GTE LOJA')
    cargo = cargo.replace('GTE LOJA II','GTE LOJA')
    cargo = cargo.replace('SUPV VENDAS I','SUPV VENDAS')
    cargo = cargo.replace('SUPV VENDASI','SUPV VENDAS')
    cargo = cargo.replace('SUPV VENDAS II','SUPV VENDAS')
    return cargo

bd['cargo_generico'] = bd['Cargo'].apply(substituir_cargos)
